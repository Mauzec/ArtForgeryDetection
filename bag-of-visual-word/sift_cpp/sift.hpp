...
find_keypoint_matches(std::vector<Keypoint> &a, std::vector<Keypoint> &b,
...
void find_input_img_coords(Keypoint &kp, float offset_s, float offset_x,
                           float offset_y, float sigma_min, float min_pix_dist,
                           int n_spo) {
...
find_keypoints_and_descriptors(const Image &img, float sigma_min,
  ...
  for (Keypoint &kp_tmp : tmp_kps) {
    ...
    for (float theta : orientations) {
      ...
      kps.push_back(kp);
      jn["kpsdes"].push_back(
          {{"x", kp.x}, {"y", kp.y}, {"des", kp.descriptor}});
    }
  }
  file << setw(4) << jn << endl;
  file.close();
