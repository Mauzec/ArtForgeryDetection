...
from cv2 import resize, GaussianBlur, subtract, KeyPoint, INTER_LINEAR, INTER_NEAREST, INTER_AREA
...
class DescriptorSift:
    ''' SIFT Descriptor class. Use static method calculate_keypoints_N_desctiptions for receive keypoints and descriptors.
        You do not need initialize a class object'''
    ...
    def generate_gaussian_images(image, count_octaves, gauss_kernels):
        ...
        for octave_idx in range(count_octaves):
            ...
            for gauss_kernel in gauss_kernels[1:]:
                gauss_images_in_octave.append(GaussianBlur(image, (0, 0), sigmaX=gauss_kernel, sigmaY=gauss_kernel))
            gauss_images.append(gauss_images_in_octave)
    ...
    def localize_extremum_quadrafit(x, y, image_idx, octave_idx, intervals_count, dog_images_in_octave, sigma, contrast_threshold, image_border_width, eigenvalue_ratio=10, attempts_until_convergence=5):
        ...
        for attempt_idx in range(attempts_until_convergence):
            ...
            gradient = DescriptorSift.calculate_gradient_at_center_pixel(cube)
            hessian = DescriptorSift.calcualte_hessian_at_center_pixel(cube)
    ...
    def calculate_keypoints_with_orientations(keypoint, octave_idx, gauss_image, radius_factor=3, bins_count=36, peak_ratio=.8, scale_factor=1.5):
        # print('[DEBUG] calculate_keypoints_with_orientions')
        ...
        for peak_index in orientation_peaks:
            ...
            if peak_value >= peak_ratio * orientation_max:
                ...
                if abs(orientation - 360.) < DescriptorSift.EPS():
                    orientation = 0
    ...
    def search_scale_space_extrema(gauss_images, dog_images, intervals_count, sigma, image_border_width, contrast_threshold=.04):
        ...
        for octave_idx, dog_images_in_octave in (enumerate(tqdm(dog_images))):
            for image_idx, (img1, img2, img3) in enumerate(zip(dog_images_in_octave, dog_images_in_octave[1:], dog_images_in_octave[2:])):
                ...
                for x in range(image_border_width, img1.shape[0] - image_border_width):
                    for y in range(image_border_width, img1.shape[1] - image_border_width):
                        if DescriptorSift.is_pixel_an_extremum(img1[x-1:x+2, y-1:y+2], img2[x-1:x+2, y-1:y+2], img3[x-1:x+2, y-1:y+2], threshold):
                            local_result = DescriptorSift.localize_extremum_quadrafit(x, y, image_idx + 1, octave_idx, intervals_count, dog_images_in_octave, sigma, contrast_threshold, image_border_width)
                            if local_result is not None:
                                keypoint, localized_image_idx = local_result
                                keypoints_with_orientations = DescriptorSift.calculate_keypoints_with_orientations(keypoint, octave_idx, gauss_images[octave_idx][localized_image_idx])
    ...
    def remove_dublicate_keypoints(keypoints):
        ...
        keypoints.sort(key=cmp_to_key(DescriptorSift.compare_keypoints))
    ...
    def generate_descriptors(keypoints, gaussian_images, window_width=4, num_bins=8, scale_multiplier=3, descriptor_max_value=0.2):
        ...
        for keypoint in keypoints:
            octave, layer, scale = DescriptorSift.unpack_octave(keypoint)
            ...
            descriptor_vector /= max(norm(descriptor_vector), DescriptorSift.EPS())
    ...
    def calculate_keypoints_N_desctiptions(image, sigma=1.6, intervals_count=3, assumed_blur=.5, image_border_width=5):
        ...
        gauss_images = DescriptorSift.generate_gaussian_images(base_image, octaves_count, gauss_kernels)
        dog_images = DescriptorSift.generate_DOG_images(gauss_images)
        keypoints = DescriptorSift.search_scale_space_extrema(gauss_images, dog_images, intervals_count, sigma, image_border_width)
        keypoints = DescriptorSift.remove_dublicate_keypoints(keypoints)
        keypoints = DescriptorSift.convert_keypoints_to_input_image_size(keypoints)
        descriptions = DescriptorSift.generate_descriptors(keypoints, gauss_images)
        return keypoints, descriptions
    
    @staticmethod
    def compute(image): return DescriptorSift.calculate_keypoints_N_desctiptions(image)